...
from app.controllers.feature_controller import ProFeatureController
from app.menu_config import PRO_MENU
...
class ProApp:
    ...
    def _init_views(self):
        self.features = ProFeatureController(
            self.root,
            self.translator,
            self.processor,
    ...
    def _load_background(self):
        ...
        path = self.asset_path("bg_pro.png")
    ...
    def _license_gate(self):
        ...
        if cache:
            ...
            if key and self.license.verify(key):
                return True

        win = tk.Toplevel(self.root)
        win.title("Proライセンス認証")
        ...
        tk.Label(
            win,
            text="ライセンスキーを入力してください",
            bg=DARK,
            fg=SUB,
            font=("Segoe UI", 11)
        ).pack()

        entry = tk.Entry(win, width=48, font=("Segoe UI", 11))
        entry.pack(pady=12)
    ...
    def _build_drop_area(self):
        ...
        drop_card.place(relx=0.24, rely=0.31, relwidth=0.40, relheight=0.16)
    ...
    def _build_status_panel(self):
        stats_card = self._scrollable_card(0.73, 0.18, 0.19, 0.22)

        self.label(stats_card, "STATUS", size=12, bold=True).pack(anchor="w", padx=13, pady=(10, 6))
        ...
        self.dict_count = self._mini_stat(stats_card, "辞書", "0")

    def _build_action_panel(self):
        action_card = self._scrollable_card(0.73, 0.42, 0.19, 0.51)

        self.label(action_card, "LAUNCHER MENU", size=11, bold=True).pack(anchor="w", padx=13, pady=(10, 7))

        for text, method_name, kind in PRO_MENU:
            style = "Launcher.Accent.TButton" if kind == "accent" else "Launcher.Dark.TButton"
    ...
    def _build_log_panel(self):
        ...
        log_card.place(relx=0.05, rely=0.75, relwidth=0.52, relheight=0.15)

        self.label(log_card, "AI TRANSLATION LOG", size=11, bold=True).pack(anchor="w", padx=12, pady=(7, 2))
    ...
    def _mini_stat(self, parent, title, value):
        ...
        tk.Label(row, text=title, bg=GLASS, fg=SUB, font=("Segoe UI", 9, "bold"), width=10, anchor="w").pack(side="left")

        label = tk.Label(row, text=value, bg=GLASS, fg=ACCENT, font=("Segoe UI", 15, "bold"))
        label.pack(side="right")

        return label

    def _write_welcome(self):
        self.log_text.insert("end", "✨ MCAddon Translator PRO 起動\n")
        self.log_text.insert("end", "✓ .mcaddon / .mcpack / .zip 選択対応\n")
        self.log_text.insert("end", "✓ 画面全体D&D対応\n")
        self.log_text.insert("end", "✓ 背景優先ランチャーUI\n\n")
    ...
    def preview_window(self):
        self.features.preview_window()

    def translation_history(self):
        self.features.translation_history()

    def dictionary_editor(self):
        self.features.dictionary_editor()

    def translation_checker(self):
        self.features.translation_checker()

    def addon_diagnostics(self):
        self.features.addon_diagnostics()

    def ai_review(self):
        self.features.ai_review()

    def json_repair(self):
        self.features.json_repair()

    def translation_rate(self):
        self.features.translation_rate()

    def quality_check(self):
        self.features.quality_check()

    def cloud_dictionary(self):
        self.features.cloud_dictionary()

    def translation_report(self):
        self.features.translation_report()

    def quality_score(self):
        self.features.quality_score()
    ...
    def license_settings(self):
        messagebox.showinfo("ライセンス", "PRO認証済みです")
    ...
    def select_input_path(self):
        ...
        folder = filedialog.askdirectory(title="または翻訳するフォルダを選択")
        return folder
    ...
    def update_stats(self):
        ...
        self.dict_count.config(text=str(stats.get("dictionary", 0)))
        ...
        try:
            ...
            if "402" in joined or "Payment Required" in joined:
                return (
                    ...
                    "有料AIを続けて使う場合は、AIエンジン状態画面から料金ページを開き、入金または支払い設定を行ってください。\n"
                    "無料で使いたい場合は、設定画面から Argos または Ollama に変更できます。"
                )

            if "OFFLINE" in joined or "Argos" in joined or "not installed" in joined.lower():
                return (
                    "AI翻訳エンジンが未導入、または利用できません。\n\n"
                    "無料で使いたい場合は、設定画面で Argos または Ollama を選び、"
                    "AIエンジン状態画面から必要なものをインストールしてください。"
                )
